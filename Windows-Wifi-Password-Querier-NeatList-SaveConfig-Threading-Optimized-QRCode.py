...
class Main(object):
    ...
    def show_wifi_qr_menu(self, WlanName, WlanPassword):
        """显示WiFi二维码菜单"""
        while True:
            print("\n" + "-"*50)
            ...
            try:
                ...
                if 0 <= index < len(WlanName):
                    ...
                    # 重新显示WiFi列表
                    self.main(WlanName, WlanPassword)
                else:
                    print("无效的序号，请重新输入")
            except ValueError:
                print("请输入有效的数字")
    ...
    def Read_Wifi_Config(self,WifiName):
        if not os.path.exists(JSON_FILE_PATH):#判断是否存在Config，如果没有则先使用老方法先显示WiFi密码，然后在保存到Config内
            ...
            if JsonWifiName == WifiName:
                ...
                # 添加二维码菜单
                self.show_wifi_qr_menu(WifiName, JsonWifiPassword)
                pass
            else:
            ...
            if WlanPassWordList != JsonWifiPassword:#如果有一个信息错误，便会修改json中的信息并更新界面
                ...
                self.Write_Wifi_Config(WifiName,WlanPassWordList)
                # 添加二维码菜单
                self.show_wifi_qr_menu(WifiName, WlanPassWordList)
                pass
    ...
    def OldWay(self):
        ...
        # 添加二维码菜单
        self.show_wifi_qr_menu(WlanNameList, WlanPassWordList)
